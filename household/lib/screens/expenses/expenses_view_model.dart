...
class ExpensesViewModel extends ChangeNotifier {
  ...
  void updateCategoryInList(Category updated) {
    categories = categories.map((c) {
      ...
      final updatedSubs = c.subCategories.map((s) => s.id == updated.id ? updated : s).toList();
      if (identical(updatedSubs, c.subCategories)) return c;
      return Category(
        id: c.id, name: c.name, nameHe: c.nameHe, color: c.color, icon: c.icon,
        parentCategoryId: c.parentCategoryId, subCategories: updatedSubs,
      );
    }).toList();
    notifyListeners();
  }
