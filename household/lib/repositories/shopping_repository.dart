...
class ShoppingRepository {
  ...
  /// Mark a session as completed and create an expense from its totals.
  /// Returns the (session, expense) pair from the server.
  Future<({ShoppingSession session, Expense expense})> completeSession(
      int sessionId, Map<String, dynamic> body) async {
    final res = await _dio.post(
        '/app/shopping/sessions/$sessionId/complete',
        data: body);
    return (
      session: ShoppingSession.fromJson(res.data['session'] as Map<String, dynamic>),
      expense: Expense.fromJson(res.data['expense'] as Map<String, dynamic>),
    );
  }

  /// Upsert a BudgetPlanItem from the session's plan fields and link the two.
  Future<ShoppingSession> attachToPlan(int sessionId) async {
    final res = await _dio.post(
        '/app/shopping/sessions/$sessionId/attach-to-plan');
    return ShoppingSession.fromJson(res.data['session'] as Map<String, dynamic>);
  }
}
